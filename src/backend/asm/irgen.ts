import { AST, ASTNode, INSTRUCTIONS, IR, OPERAND_TYPES } from "../types/asm.types"
...
export function generateIR(ast: AST): IR {
    ...
    for (const astNode of ast) {
        console.log('generateIR astNode:', JSON.stringify(astNode, null, 2));
        const mnemonic = astNode.mnemonic;

        if (mnemonic === 'DB') {
            for (const operand of astNode.operands) {
                console.log('generateIR operand:', JSON.stringify(operand, null, 2));
        ...
        switch (info.format) {
            ...
            case 'J':
                ...
                if (!targetAddress) {
                    throw new Error(`Target label for J instruction ${mnemonic} not found: ${targetLabel}`);
                }
